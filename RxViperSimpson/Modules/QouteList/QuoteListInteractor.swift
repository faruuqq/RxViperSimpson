...
class QuoteListInteractor {
    ...
    private let apiManager = ApiManager(withEndpoint: "https://thesimpsonsquoteapi.glitch.me/quotes?count=10")
    ...
    func fetchQuote() {
        apiManager.getData()
            .subscribe { [weak self] (quotes: [QuoteListEntity]) in
                self?.quoteLists.onNext(quotes)
                self?.quoteLists.onCompleted()
            } onError: { [weak self] err in
                print("error from quote list interactor with \(err)")
                self?.quoteLists.onError(err)
            }
            .disposed(by: bag)
    }
